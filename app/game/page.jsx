...
const QuizPage = () => {
  ...
  const handleAnswerClick = (answer) => {
    if (questionTimer !== '00:00') {
      ...
      // Check if it's the last question, then redirect to result page
      if (questionNumber === 10) {
        dispatch(nextQuestion());
        if (questionTimer !== '00:00') {
          // Navigate to result page if the user answers before the timer ends
          window.location.href = '/result';
        }
      } else {
        dispatch(nextQuestion());
  ...
  useEffect(() => {
    if (questionTimer === '00:00') {
      ...
      if (questionNumber < 10) {
        ...
        // Redirect to result page when all questions are answered
        window.location.href = '/result';
      }
    }
  }, [questionTimer, questionNumber, dispatch, timerInterval]);
        ...
        >
          {['Eiffel Tower', 'Moai', 'Taj Mahal', 'Kaaba'].map((answer) => (
            <motion.button
              ...
              style={buttonStyle(answer)}
              whileTap={{ scale: 0.95 }}
              disabled={questionTimer === '00:00'} // Disable button when timer reaches '00:00'
            >
              {answer}
            </motion.button>
          ))}
